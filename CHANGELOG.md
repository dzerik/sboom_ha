...
### Added
- **Опция «Караоке-закраска»** (Options Flow) — посимвольная подсветка строки по мере пропевания. **По умолчанию выключена**: строка показывается статично. Когда off — доля пропетости (`frac`) не идёт ни в рендер, ни в кэш-ключ стрима, так что лишних кадров нет.

### Known limitations
- Закраска рассчитана на письмо слева-направо; для RTL (арабский/иврит) направление заливки не инвертируется (нужны bidi + shaping, напр. libraqm) — отдельная задача. Т.к. закраска off по умолчанию, на поведение «из коробки» не влияет.

## [0.28.0]

### Added
- **Заглушка-фон вместо чёрного экрана**, когда обложка не нашлась (редкий BT-трек, радио без арта, сбой загрузки). Бандл из 4 свободных градиентов (StockSnap/Flickr, **CC0 1.0** — атрибуция не требуется, см. `backgrounds/CREDITS.txt`). Выбор стабильно-случайный по треку (`title|artist` → индекс) — фон не мигает от кадра к кадру, но у разных треков разный. Применяется и к караоке-кадру, и к idle. Общий вес ассетов ~64 КБ.

## [0.27.0]

### Added
- **Обложка для Bluetooth и радио по исполнителю+треку.** У некаталожного контента нет Zvuk-`release_id`, поэтому обложки не было (карточка и караоке-фон пустые). Теперь ищется по `title`+`artist` в публичных Search API без ключей: **iTunes** (апскейл до 600×600) → **Deezer** (резерв). Волатильно (один in-memory слот на трек, без диск-кэша). Подхватывается и в `media_image_url` (карточка media_player), и в blur-фон караоке-камеры. Радио тоже получает обложку текущей песни (обложке синк не нужен).
- Кэш обложки в караоке-камере переведён с `track_id` на URL — раньше некаталожные треки (без `track_id`) делили один ключ.

## [0.26.1]

### Fixed
- **Радио исключено из караоке.** Колонка отдаёт для радио позицию ЭФИРА (напр. 1224 c при песне 36–250 c), а не песни — синхронизировать лирику нечем. Лирика для радио больше не запрашивается, караоке-камера показывает idle (обложка/заголовок), а не замороженную последнюю строку. Bluetooth (позиция песенная) — работает и синхронизируется корректно (подтверждено вживую).
- Диагностический сенсор `_lyrics` больше не показывает `no_track` для Bluetooth: теперь состояние лирики отражается и для некаталожного контента (у которого нет Zvuk `track_id`).

## [0.26.0]

### Added
- **Караоке/лирика для Bluetooth и радио** — тянется напрямую из провайдеров (Lrclib → NetEase), **без персистентного диск-кэша**. У BT/радио нет глобального каталожного `track_id` (он есть только у контента Zvuk), поэтому:
  - ключ — синтетический `title|artists`; кэш каталога (по `track_id`) не затрагивается и не засоряется;
  - один in-memory слот на текущий трек — API не дёргается каждый poll, но и на диск ничего не пишется (BT — почти всегда одноразовая чужая музыка, радио — эфемерно);
  - контент без исполнителя (радио-реклама, джинглы) не запрашивается (гейт по `artists`).
  - **BT** синхронизируется по позиции/длительности от колонки; у **радио** позиция эфирная, поэтому синк приблизительный (сам текст показывается).
...
### Changed
- **Честная документация op 18 (keep-alive).** Эксперимент показал: транспортный WS ping/pong держит сессию сам (≥150 c idle без op 18 — соединение живо), а keep-alive-семантика op 18 нашим фаззингом не подтверждена. Комментарии в коде уточнены: op 18 оставлен как дешёвая application-level страховка, но его вклад в поддержание связи не изолирован. Поведение не менялось.
