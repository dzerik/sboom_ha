...
- Оптимистичные кнопки транспорта (мгновенная реакция); дедуп play-команд; «потерянные/неверные» тапы устранены (shouldUpdate-гейт, захват трека на pointerdown, приручённый рефетч очереди) — по итогам глубокого аудита.
- Убран мусорный `playlist_title` из now-playing.
