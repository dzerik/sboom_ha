...
def test_lyrics_cache_data_serialises_and_skips_none():
    ...
    coord.lyrics_by_track = {"1": _lyrics("a"), "2": None, "3": _lyrics("b")}
    data = coord._lyrics_cache_data()
...
async def test_load_lyrics_cache_populates_from_store():
    coord = build_coordinator()
    coord._lyrics_store._data = {
        "42": {"plain": "hello", "synced": None,
               "instrumental": False, "source": "lrclib"},
    }
    await coord._load_lyrics_cache()
    assert coord.lyrics_by_track["42"].plain == "hello"
...
async def test_load_lyrics_cache_skips_garbage_entries():
    ...
    await coord._load_lyrics_cache()
    assert "ok" in coord.lyrics_by_track
    assert "bad" not in coord.lyrics_by_track
